...
namespace DNexusReadHelper
{
  ...
  template<class T>
      ...
      if(err<0)
	{
      ...
      htri_t unitsExists = H5Aexists_by_name(dataid, ".", "units", H5P_DEFAULT);
	...
	{
	  hid_t attr_id = H5Aopen_by_name(dataid, ".", "units", H5P_DEFAULT, H5P_DEFAULT);
  ...
  bool CreateLUT(hid_t fid, std::map<size_t, std::string> *lut) 
  {
    ...
    while(true)
      {
	char dataName[1024];
	sprintf(dataName, "data_%06d", linkCnt++);
